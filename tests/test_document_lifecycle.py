...
from pipeline.models import DocumentQueryEnabledUpdate, ChunkUpdate, ChunkQueryEnabledUpdate
...
# =============================================================================
# Per-chunk query enable/disable (mirrors the doc-level toggle)
# =============================================================================


def test_chunk_query_enabled_route_requires_admin(lifecycle_indexed_doc, monkeypatch):
    """Curator is 403; admin flips the chunk off and purges Marqo (fail-closed)."""
    calls = []
    ...
    with pytest.raises(HTTPException) as exc:
        _run(
            api.set_chunk_query_enabled(
                lifecycle_indexed_doc,
                ChunkQueryEnabledUpdate(enabled=False),
                _curator_in("tenant-a"),
                chunk_num=1,
            )
        )
    assert exc.value.status_code == 403
    assert calls == []  # curator never reached the purge
    ...
    assert bool(updated["is_excluded"]) is True
    # Dropped from Marqo via the doc's own resolved index (never bare default).
    assert len(calls) == 1
    assert calls[0]["index_name"] == "t-tenant-a-vet"
    assert calls[0]["chunk_num"] == 1
    # Dropped from search: default chunk read excludes it.
    visible = [c["chunk_number"] for c in db_mod.get_chunks(lifecycle_indexed_doc, include_excluded=False)]
    assert 1 not in visible
    assert 2 in visible


def test_chunk_query_enabled_missing_chunk_404(lifecycle_indexed_doc, monkeypatch):
    monkeypatch.setattr(api, "delete_single_chunk_from_marqo", lambda *a, **k: {"deleted": False})
    with pytest.raises(HTTPException) as exc:
        _run(
            api.set_chunk_query_enabled(
                lifecycle_indexed_doc,
...
def test_chunk_query_enabled_502_before_flip_on_marqo_error(lifecycle_indexed_doc, monkeypatch):
    """A failed Marqo purge must 502 and leave the chunk NOT excluded — never
    hidden-but-still-searchable (mirror set_document_query_enabled ordering)."""
    monkeypatch.setattr(
        api, "delete_single_chunk_from_marqo", lambda *a, **k: {"deleted": False, "error": "marqo down"}
    )
    ...
    # DB was NOT flipped — purge failed before any state change.
    assert bool(db_mod.get_chunk(lifecycle_indexed_doc, 1)["is_excluded"]) is False


def test_chunk_query_reenable_sets_reindex_required(lifecycle_indexed_doc, monkeypatch):
    """Re-enabling a disabled chunk clears exclusion and marks the doc reindex_required."""
    monkeypatch.setattr(
        api, "delete_single_chunk_from_marqo", lambda *a, **k: {"deleted": True, "chunk_id": "c1"}
    )

    # First disable chunk 1.
    _run(
        api.set_chunk_query_enabled(
            lifecycle_indexed_doc,
            ChunkQueryEnabledUpdate(enabled=False),
    ...
    assert int(db_mod.get_document(lifecycle_indexed_doc).get("reindex_required") or 0) == 0

    updated = _run(
        api.set_chunk_query_enabled(
            lifecycle_indexed_doc,
            ChunkQueryEnabledUpdate(enabled=True),
            _admin_in("tenant-a"),
            chunk_num=1,
        )
    )
    assert bool(updated["is_excluded"]) is False
    assert int(db_mod.get_document(lifecycle_indexed_doc)["reindex_required"]) == 1
