...
@app.post("/debug/seed-test-documents")
async def seed_test_documents():
    """
    Create test documents at each workflow stage for UI testing.
    These are SQLite-only entries (no actual Temporal workflows).
    """
    stages = [
        ("registered", "01_registered_doc.pdf", 0, 0),
        ("ocr_processing", "02_ocr_processing_doc.pdf", 0, 0),
        ("ocr_review", "03_ocr_review_doc.pdf", 15, 0),
        ("chunking", "04_chunking_doc.pdf", 20, 0),
        ("chunk_review", "05_chunk_review_doc.pdf", 25, 45),
        ("ready_for_ingestion", "06_ready_for_ingestion_doc.pdf", 30, 52),
        ("ingesting", "07_ingesting_doc.pdf", 35, 60),
        ("completed", "08_completed_doc.pdf", 40, 70),
        ("failed", "09_failed_doc.pdf", 5, 0),
    ]

    created = []
    for stage, filename, page_count, chunk_count in stages:
        workflow_id = f"test-{stage}"
        document_id = hashlib.md5(workflow_id.encode()).hexdigest()

        error_msg = "OCR failed: API rate limit exceeded" if stage == "failed" else None

        db.upsert_document(
            workflow_id=workflow_id,
            document_id=document_id,
            filename=filename,
            filepath=f"minio://documents/test/{filename}",
            stage=stage,
...
@app.delete("/debug/clear-test-documents")
async def clear_test_documents():
    """Remove all test documents created by seed-test-documents."""
    stages = ["registered", "ocr_processing", "ocr_review", "chunking",
              "chunk_review", "ready_for_ingestion", "ingesting", "completed", "failed"]

    deleted = []
    for stage in stages:
        workflow_id = f"test-{stage}"
        db.delete_document(workflow_id)
