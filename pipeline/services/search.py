...
def rerank_hits(query: str, hits: list[dict], rerank_mode: str) -> list[dict]:
    ...
    text_scores = [token_overlap_score(query, str(hit.get("text") or "")) for hit in hits]
    meta_scores = [token_overlap_score(query, metadata_blob(hit)) for hit in hits]

    rescored: list[dict] = []
    if mode == "bm25lite":
        docs = [f"{str(hit.get('text') or '')} {metadata_blob(hit)}".strip() for hit in hits]
        bm_scores = bm25lite_scores(query, docs)
        ...
        sem_rank = rank_desc(semantic_scores)
        text_rank = rank_desc(text_scores)
        meta_rank = rank_desc(meta_scores)
        k = 30
...
def run_search(
    *,
    ...
    effective_query = prepare_query_for_e5(expanded_query) if use_e5_prefix else expanded_query
